                ...
                <div class="card card-stepper mb-4 shadow-sm" style="border-radius: 16px; background: #ffffff;">
                    <div class="card-body p-5">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h5 class="mb-0">📄 INVOICE <span class="text-primary font-weight-bold">#{{$invoice_code}}{{ $order->id }}</span></h5>
                            <p class="mb-0">📅 Your Date Orders: <strong>{{ \Carbon\Carbon::parse($order->expected_arrival)->format('Y-m-d') ?? 'TBD' }}</strong></p>
                            <p class="mb-0">📅 Expected Arrival: <strong>{{ \Carbon\Carbon::parse($order->expected_arrival)->addDay(4)->format('Y-m-d') ?? 'TBD' }}</strong></p>
                        </div>
                        ...
                        <div class="progress-container mb-4">
                            <div class="progress" style="background: #e9ecef; border-radius: 50px; height: 12px; position: relative;">
                                <div class="progress-bar" style="width: {{ $order->status == 'new' ? '25%' : ($order->status == 'processing' ? '50%' : ($order->status == 'shipped' ? '75%' : '100%')) }}; height: 100%; border-radius: 50px; background: linear-gradient(90deg, #4e54c8, #8f94fb); transition: width 0.6s ease;"></div>

                                <div class="progress-icons" style="position: absolute; top: -20px; left: 0; right: 0; display: flex; justify-content: space-between;">
                                    <div class="step {{ in_array($order->status, ['new', 'processing', 'shipped', 'delivered']) ? 'active' : '' }}">🛒</div>
