            ...
            <div class="col-lg-8">
                <div class="card shadow-lg border-0 rounded-4">
                    <div class="card-body p-4">
                        <h5 class="mb-3"><i class="bi bi-cart3"></i> Cart Items (<span
                                id="cart-count">{{ $cartItems->count() }}</span>)</h5>
            ...
            <div class="col-lg-4">
                <div class="card shadow-lg border-0 rounded-4 sticky-top">
                    <div class="card-body p-4">
                        ...
                        <hr>
                        <a href="{{ url('/checkout') }}" class="btn btn-outline-primary    w-100"><i
                                class="bi bi-bag-check-fill"></i> Proceed to Checkout</a>
                    ...
                    if (result.isConfirmed) {
                        $.post("{{ route('frontend.cart.remove') }}", {
                            ...
                            if (response.success) {
                                ...
                                $(".cart-item[data-id='" + cartId + "']").fadeOut(500,
                                    function() {
                                        $(this).remove();

                                        // Update grand total with value from server
                                        $("#grand-total").html(
                                            `<strong>$${parseFloat(response.grandTotal).toFixed(2)}</strong>`
                                            );

                                        // Update cart count
                                        var currentCount = parseInt($("#cart-count")
                                            .text());
                                        $("#cart-count").text(currentCount - 1);
                                    });
                                Swal.fire("Removed!", "Product removed from cart.",
                                    "success");
                            } else {
